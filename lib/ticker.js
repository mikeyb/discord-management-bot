            ...
            return rp(tickerOptions).then(
                response => {
                    ...
                    if (response.status.error_code === 0) {
                        ...
                        change24h = change24hSplit[0] + '.' + change24hSplit[1].substr(0,2);
                        
                        let change7d = '' + response.data[tokenId].quote.USD.percent_change_7d;
                        const change7dSplit = change7d.split('.');
                        change7d = change7dSplit[0] + '.' + change7dSplit[1].substr(0,2);

                        return message.channel.send(
                            {
                                'embed': {
                                    'color': 0x0099ff,
                                    'title': tokenSymbol.toUpperCase() + ' Price - https://CoinMarketCap.com',
                                    'fields': [
                                        {
                                            'name': 'Price',
                                            'value': '$' + price,
